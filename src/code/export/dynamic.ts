...
import { getShouldClipToParent } from "./dynamic/shapes/clipping.ts";
    ...
    taggedNodes
        ...
        .forEach((node) => {
            if (
                ...
                const shouldClip = getShouldClipToParent(node);

                // Use parent box if the node should clip to parent
                if (
                    parent &&
                    (parent.type == "FRAME" || parent.type == "INSTANCE") &&
                    (shouldClip || shouldClip == null)
                ) {
                    box = parent.absoluteBoundingBox;
                } else {
