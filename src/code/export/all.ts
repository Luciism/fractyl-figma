...
export default async function completeExport(masterNode: SceneNode) {
    ...
    const layoutSchemas = await Promise.all(
        exportSettings
            .scales
            .filter(scale => scale.isEnabled && scale.scale > 0 || scale.isDefault)
            // Eliminate duplicate scale names
            .filter((scale, i, scales) => scales.findIndex(s => s.name === scale.name) === i || scale.isDefault)
            .map(async (scale, i) => {
